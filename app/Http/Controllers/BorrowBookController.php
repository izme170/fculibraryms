...
class BorrowBookController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        if($status != 'all'){
            if($status == 'returned'){
                $query = BorrowedBook::whereNotNull('returned');
            }elseif($status == 'borrowed'){
                $query = BorrowedBook::whereNull('returned')->where('due_date', '>=', now());
        ...
        $borrowed_books = $query->orderBy($sort, $direction)->paginate(10);
